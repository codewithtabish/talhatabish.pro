...
const WAITLIST_TEXT = {
  en: {
    ...
    badge: "Be the first to join!",
  },
  ur: {
    button: "ویٹ لسٹ میں شامل ہوں",
    placeholder: "اپنا ای میل درج کریں",
    badge: "سب سے پہلے شامل ہوں!",
  },
  ar: {
    button: "انضم إلى قائمة الانتظار",
    placeholder: "أدخل بريدك الإلكتروني",
    badge: "كن أول المنضمين!",
  },
  zh: {
    button: "加入候补名单",
    placeholder: "输入您的邮箱",
  ...
  fr: {
    ...
    badge: "Soyez le premier à rejoindre !",
  },
  de: {
...
const users = [
  { imgUrl: 'https://avatars.githubusercontent.com/u/111780029', alt: 'User 1' },
  { imgUrl: 'https://avatars.githubusercontent.com/u/123104247', alt: 'User 2' },
  { imgUrl: 'https://avatars.githubusercontent.com/u/115650165', alt: 'User 3' },
  { imgUrl: 'https://avatars.githubusercontent.com/u/71373838', alt: 'User 4' },
];

const WaitlistComp: React.FC<Props> = ({ locale = 'en' }) => {
  const content = waitlistContent[locale] || waitlistContent['en'];
  const { button, placeholder, badge } = WAITLIST_TEXT[locale] || WAITLIST_TEXT['en'];
  ...
  // SVG icon for badge (for SEO, not a logo)
  const BadgeSVG = (
    <svg
      width="20"
      height="20"
    ...
    >
      <circle cx="16" cy="16" r="16" fill="#e60a64" />
      <text
        x="50%"
        y="55%"
        textAnchor="middle"
      ...
      <main className="relative items-center justify-center overflow-hidden">
        ...
        <div className="relative z-[100] mx-auto w-full max-w-7xl px-2 sm:px-6 py-10 text-center">
          ...
          <motion.div
            ...
            className={cn(
              "mb-8 flex items-center justify-center gap-2 rounded-full border border-primary/10 bg-gradient-to-r from-primary/15 to-primary/5 px-4 py-2 backdrop-blur-sm w-auto max-w-fit mx-auto",
              isRtl ? "flex-row-reverse" : ""
            )}
            dir={isRtl ? "rtl" : "ltr"}
            aria-label={badge}
            style={{
              marginTop: "2.5rem", // Push badge down so it never overlaps language switcher
              zIndex: 10,
              position: "relative",
              width: "fit-content",
              minWidth: 0,
          ...
          {/* Heading (only one h1 for SEO) */}
          <motion.h1
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.2 }}
            className={cn(
              'mb-4 cursor-crosshair bg-gradient-to-b from-foreground via-foreground/80 to-foreground/40 bg-clip-text text-3xl sm:text-5xl lg:text-7xl font-bold text-transparent',
              brico.className,
          ...
          <motion.p
            ...
            className={cn(
              "mb-12 mt-2 text-muted-foreground text-base sm:text-lg",
              isRtl ? "text-right" : "text-center"
            )}
            dir={isRtl ? 'rtl' : 'ltr'}
          >
          ...
          <motion.form
            ...
            className={cn(
              "mx-auto flex flex-col gap-4 sm:flex-row max-w-xl w-full",
              isRtl ? "flex-row-reverse" : ""
            )}
            dir={isRtl ? 'rtl' : 'ltr'}
          ...
          >
            <div className={`relative flex-1 min-w-0 ${isRtl ? 'text-right' : ''}`}>
              <motion.input
                key="email-input"
                ...
                className={cn(
                  ...
                  isRtl ? "text-right placeholder:text-right" : ""
                )}
          ...
          <motion.div
            ...
            className={cn(
              "mt-10 flex items-center justify-center gap-1",
              isRtl ? "flex-row-reverse" : ""
            )}
          >
            ...
            <motion.span
              ...
              className={cn("ml-2 text-muted-foreground", isRtl ? "ml-0 mr-2" : "")}
            >
              <span className="font-semibold text-primary">100+</span>{" "}
              {locale === 'ur'
                ? 'پہلے ہی شامل ہو چکے ہیں ✨'
                : locale === 'ar'
                ? 'انضموا بالفعل ✨'
                : 'already joined ✨'}
            </motion.span>
